...
func TestSpecificUnexistantIntefaceInfo(t *testing.T) {
	...
	ni := NewNetworkInfo()
	_, err := ni.GetAdaptersInfo(pkg.NICSearchFilterGUID("{??-3A1C-42FB-9E5C-0C2C440AB463}"))
	if err == nil {
...
func TestSpecificAdapterInfoFromGUID(t *testing.T) {
	...
	ni := NewNetworkInfo()
	adapters, err := ni.GetAdaptersInfo(pkg.NICSearchFilterGUID("{6ABE2201-1694-4D6A-9B93-5CCD136E68D0}"))
	if err != nil {
		t.Fatal(err)
	}
	var b []byte
...
func TestSpecificAdapterInfoFromIndex(t *testing.T) {
	...
	ni := NewNetworkInfo()
	adapters, err := ni.GetAdaptersInfo(pkg.NICSearchFilterID(14))
	if err != nil {
		t.Fatal(err)
	}
	var b []byte
...
func TestWifiAdapters(t *testing.T) {
	...
	ni := NewNetworkInfo()
	adapters, err := ni.GetAdaptersInfo(pkg.NicType(pkg.NIC_WIRELESS802_11))
	if err != nil {
		t.Fatal(err)
	}
	var b []byte
...
func TestMoreFilters(t *testing.T) {
	...
	ni := NewNetworkInfo()
	adapters, err := ni.GetAdaptersInfo(pkg.NicType(pkg.NIC_WIRELESS802_11), pkg.NICSearchFilterID(14))
	if err != nil {
		t.Fatal(err)
	}
	var b []byte
